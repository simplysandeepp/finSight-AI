...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['revenue_ttm'] = rolling_rev.sum().reset_index(level=0, drop=True)
    ...
    # Clean inf values generated by pct_change divides by zero
    import numpy as np_local
    df.replace([np_local.inf, -np_local.inf], np_local.nan, inplace=True)
    return df
...
def main():
    ...
    # Prepare manifest
    feature_list = [col for col in df_featured.columns if col not in df.columns or col.startswith(('scenario_', 'sector_', 'analyst_', 'revenue_ttm'))]
    manifest = {
        "version": "v1.0.0",
